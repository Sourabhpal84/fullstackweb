...
function normalizeOfflineCart(cart = []) {
  if (!Array.isArray(cart)) return [];
  return cart.map(item => ({
    dishId: cleanString(item.dishId || item.id, 120),
...
async function priceOfflineCart(cart = []) {
  const normalized = normalizeOfflineCart(cart);
  if (!normalized.length) throw Object.assign(new Error("Cart is empty"), { status: 400 });
  const pricedItems = [];
  for (const item of normalized) {
    const snap = await db.collection("dishes").doc(item.dishId).get();
    if (!snap.exists) throw Object.assign(new Error(`${item.name || "An item"} is no longer available`), { status: 409 });
    const dish = snap.data() || {};
    if (dish.available === false || dish.active === false) {
      throw Object.assign(new Error(`${dish.name || item.name || "An item"} is currently unavailable`), { status: 409 });
    }
    const price = Number(dish.price || dish.finalPrice || dish.offerPrice || 0);
    if (!Number.isFinite(price) || price <= 0) throw Object.assign(new Error(`${dish.name || item.name || "An item"} has invalid pricing`), { status: 409 });
    pricedItems.push({
      dishId: item.dishId,
      name: cleanString(dish.name || item.name, 160),
      categoryId: cleanString(dish.categoryId || dish.category || "", 120),
      qty: item.qty,
      price,
      lineTotal: Math.round(price * item.qty)
    });
  }
...
function offlineCouponDiscount(coupon = {}, subtotal = 0) {
  const type = String(coupon.type || coupon.discountType || "").toLowerCase();
  let discount = 0;
  if (coupon.freeDelivery === true) return 0;
  if (type === "percentage" || type === "percent") {
    discount = subtotal * (Number(coupon.discountValue || coupon.value || 0) / 100);
    if (Number(coupon.maxDiscount || 0) > 0) discount = Math.min(discount, Number(coupon.maxDiscount));
  } else {
    discount = Number(coupon.discountValue || coupon.value || 0);
  }
  return Math.min(subtotal, Math.max(0, Math.round(discount)));
}

async function applyOfflineCoupon(code, subtotal) {
  const couponCode = cleanString(code, 40).toUpperCase();
  if (!couponCode) return { couponCode: "", couponId: "", discount: 0 };
  const snap = await db.collection("coupons").where("code", "==", couponCode).limit(1).get();
  if (snap.empty) throw Object.assign(new Error("Coupon not found"), { status: 404 });
  const docSnap = snap.docs[0];
  const coupon = docSnap.data() || {};
  const now = Date.now();
  const expiry = coupon.expiryDate?.toMillis ? coupon.expiryDate.toMillis() : Number(coupon.expiryDate || 0);
  if (coupon.active !== true || coupon.deleted === true) throw Object.assign(new Error("Coupon is not active"), { status: 409 });
  if (expiry && expiry < now) throw Object.assign(new Error("Coupon expired"), { status: 409 });
  if (Number(coupon.usageLimit || 0) > 0 && Number(coupon.usedCount || 0) >= Number(coupon.usageLimit)) {
    throw Object.assign(new Error("Coupon usage limit reached"), { status: 409 });
  }
  if (subtotal < Number(coupon.minOrderAmount || 0)) {
    throw Object.assign(new Error(`Minimum order for this coupon is ₹${Number(coupon.minOrderAmount || 0)}`), { status: 409 });
  }
  return {
    couponCode,
    couponId: docSnap.id,
    discount: offlineCouponDiscount(coupon, subtotal)
  };
}

async function nextOfflineOrderIdentity(transaction) {
  const counterRef = db.collection("counters").doc("offlineOrders");
  const snap = await transaction.get(counterRef);
  const last = Number(snap.exists ? snap.data().last || 0 : 0) + 1;
  const now = new Date();
  const dd = String(now.getDate()).padStart(2, "0");
  const mm = String(now.getMonth() + 1).padStart(2, "0");
  const yy = String(now.getFullYear()).slice(-2);
  transaction.set(counterRef, { last, updatedAt: FieldValue.serverTimestamp() }, { merge: true });
  return {
    orderNumber: `MGZ-${dd}${mm}${yy}-${String(last).padStart(3, "0")}`,
    tokenNumber: String(1000 + (last % 9000))
  };
}

async function buildOfflineOrderPayload(body = {}, payment = {}) {
  ...
  const { items, subtotal } = await priceOfflineCart(body.items || body.cart || []);
  const coupon = await applyOfflineCoupon(body.couponCode, subtotal);
  const finalTotal = Math.max(0, subtotal - coupon.discount);
  const paymentMethod = cleanString(body.paymentMethod || payment.paymentMethod || "cash", 40).toLowerCase();
  const paid = String(payment.paymentStatus || "").toLowerCase() === "paid";
  return {
    orderType: "offline",
    orderSource: "offline_restaurant",
    customerName,
    customerPhone: cleanString(body.customerPhone || body.mobileNumber, 30),
    tableNo: cleanString(body.tableNo || body.tableNumber, 30),
    specialInstructions: cleanString(body.specialInstructions, 500),
    items,
    cart: items,
    subtotal,
    ...
    amount: finalTotal,
    amountToCollect: paid ? 0 : finalTotal,
    paymentMethod,
    paymentStatus: paid ? "paid" : "pending",
    paymentRequired: paymentMethod === "online",
    paymentCompleted: paid,
    paymentCaptured: paid,
    razorpayOrderId: cleanString(payment.razorpayOrderId, 120),
    razorpayPaymentId: cleanString(payment.razorpayPaymentId, 120),
    paymentSessionId: cleanString(payment.paymentSessionId, 160),
    orderStatus: "New Order",
    status: "New Order",
    lifecycleStatus: "offline_new_order"
  };
}

async function createOfflineOrder(body = {}, payment = {}) {
  const payload = await buildOfflineOrderPayload(body, payment);
  const orderRef = payment.orderId ? db.collection("orders").doc(String(payment.orderId)) : db.collection("orders").doc();
  let identity = {};
  let existingOrder = null;
  await db.runTransaction(async transaction => {
    const existingSnap = await transaction.get(orderRef);
    if (existingSnap.exists) {
      existingOrder = { orderId: orderRef.id, ...existingSnap.data() };
      return;
    }
    identity = await nextOfflineOrderIdentity(transaction);
    transaction.set(orderRef, {
      orderId: orderRef.id,
      ...identity,
      ...payload,
      invoiceNumber: identity.orderNumber,
      timeline: [{ status: "New Order", source: "offline_checkout", at: Date.now() }],
      createdAt: FieldValue.serverTimestamp(),
      updatedAt: FieldValue.serverTimestamp()
    }, { merge: false });
    if (payload.couponId && payload.discount > 0) {
      transaction.update(db.collection("coupons").doc(payload.couponId), {
        usedCount: FieldValue.increment(1),
        totalDiscountGiven: FieldValue.increment(payload.discount),
        lastUsedAt: FieldValue.serverTimestamp()
      });
    }
  });
  if (existingOrder) return { ok: true, ...existingOrder, duplicate: true };
  return { ok: true, orderId: orderRef.id, ...identity, ...payload };
}

exports.createOfflineOrder = onRequest(
  { region: "asia-south1", cors: allowedWebOrigins() },
  async (req, res) => {
    if (req.method === "OPTIONS") return sendJson(res, 204, {});
    if (req.method !== "POST") return sendJson(res, 405, { ok: false, error: "Method not allowed" });
    try {
      const result = await createOfflineOrder(req.body || {}, { paymentMethod: req.body?.paymentMethod || "cash", paymentStatus: "pending" });
      return sendJson(res, 200, result);
    } catch (error) {
      logger.error("createOfflineOrder failed", { error: error.message });
      return sendJson(res, error.status || 500, { ok: false, error: error.message || "Offline order failed" });
    }
  }
);

exports.createOfflinePaymentSession = onRequest(
  { region: "asia-south1", cors: allowedWebOrigins() },
  async (req, res) => {
    if (req.method === "OPTIONS") return sendJson(res, 204, {});
    if (req.method !== "POST") return sendJson(res, 405, { ok: false, error: "Method not allowed" });
    try {
      ...
      const draft = await buildOfflineOrderPayload(req.body || {}, { paymentMethod: "online" });
      const amountPaise = Math.round(Number(draft.finalTotal || 0) * 100);
      if (amountPaise < 1000) throw Object.assign(new Error("Online payment is available for orders of ₹10 or more."), { status: 400 });
      const sessionId = crypto.createHash("sha256").update(`${Date.now()}:${draft.customerName}:${amountPaise}:${Math.random()}:offline-v1`).digest("hex");
      const orderId = db.collection("orders").doc().id;
      const razorpayOrder = await getRazorpay().orders.create({
        amount: amountPaise,
        currency: "INR",
        receipt: sessionId.slice(0, 40),
        notes: { paymentSessionId: sessionId, orderId, source: "offline_restaurant" }
      });
      await db.collection("offlinePaymentSessions").doc(sessionId).set({
        id: sessionId,
        orderId,
      ...
      logger.error("createOfflinePaymentSession failed", { error: error.message });
      return sendJson(res, error.status || 500, { ok: false, error: error.message || "Offline payment session failed" });
    }
  }
);

exports.verifyOfflinePaymentAndCreateOrder = onRequest(
  { region: "asia-south1", cors: allowedWebOrigins() },
  async (req, res) => {
    if (req.method === "OPTIONS") return sendJson(res, 204, {});
    if (req.method !== "POST") return sendJson(res, 405, { ok: false, error: "Method not allowed" });
    try {
      ...
      const sessionRef = db.collection("offlinePaymentSessions").doc(String(paymentSessionId));
      const sessionSnap = await sessionRef.get();
      if (!sessionSnap.exists) throw Object.assign(new Error("Payment session not found"), { status: 404 });
      const session = sessionSnap.data() || {};
      if (session.status === "order_created" && session.createdOrderId) {
        const orderSnap = await db.collection("orders").doc(session.createdOrderId).get();
        return sendJson(res, 200, { ok: true, orderId: session.createdOrderId, ...(orderSnap.exists ? orderSnap.data() : {}) });
      }
      ...
      if (Number(payment.amount) !== Number(session.amountPaise)) throw Object.assign(new Error("Payment amount mismatch"), { status: 400 });
      ...
      const result = await createOfflineOrder(session.orderDraft || {}, {
      ...
      logger.error("verifyOfflinePaymentAndCreateOrder failed", { error: error.message });
      return sendJson(res, error.status || 500, { ok: false, error: error.message || "Offline payment verification failed" });
    }
  }
);
      ...
      if (event.event === "payment.captured" || payment.status === "captured") {
        if (paymentSessionId) {
          ...
          if (sessionSnap.exists) {
            ...
            const offlineSessionRef = db.collection("offlinePaymentSessions").doc(String(paymentSessionId));
            const offlineSessionSnap = await offlineSessionRef.get();
            if (offlineSessionSnap.exists) {
              const offlineSession = { id: offlineSessionSnap.id, ...offlineSessionSnap.data() };
              const paymentMatchesOfflineSession = Number(offlineSession.amountPaise) === Number(payment.amount)
                && offlineSession.razorpayOrderId === payment.order_id;
              if (paymentMatchesOfflineSession) {
                const result = await createOfflineOrder(offlineSession.orderDraft || {}, {
                  orderId: offlineSession.orderId,
                  paymentMethod: "online",
                  paymentStatus: "paid",
                  paymentSessionId,
                  razorpayOrderId: payment.order_id || offlineSession.razorpayOrderId,
                  razorpayPaymentId: paymentId
                });
                await offlineSessionRef.set({
                  status: "order_created",
                  createdOrderId: result.orderId,
                  razorpayPaymentId: paymentId,
                  updatedAt: FieldValue.serverTimestamp()
...
exports.adminUpdateOfflineOrderStatus = onRequest(
  { region: "asia-south1", cors: true },
  async (req, res) => {
    if (req.method === "OPTIONS") return sendJson(res, 204, {});
    if (req.method !== "POST") return sendJson(res, 405, { ok: false, error: "Method not allowed" });
    try {
      const adminUser = await requireAdmin(req);
      const orderId = cleanString(req.body?.orderId, 160);
      const requestedStatus = cleanString(req.body?.status, 40);
      const allowed = new Set(["New Order", "Preparing", "Ready", "Completed", "Cancelled"]);
      if (!orderId || !allowed.has(requestedStatus)) throw Object.assign(new Error("Valid offline order status is required"), { status: 400 });
      const orderRef = db.collection("orders").doc(orderId);
      await db.runTransaction(async transaction => {
        const snap = await transaction.get(orderRef);
        if (!snap.exists) throw Object.assign(new Error("Order not found"), { status: 404 });
        const order = snap.data() || {};
        if (order.orderType !== "offline") throw Object.assign(new Error("This is not an offline restaurant order"), { status: 400 });
        transaction.update(orderRef, {
          ...
          lastStatusUpdatedBy: adminUser.uid,
          updatedAt: FieldValue.serverTimestamp(),
          timeline: FieldValue.arrayUnion({ status: requestedStatus, source: "offline_admin", at: Date.now(), adminUid: adminUser.uid })
        });
      });
      return sendJson(res, 200, { ok: true, orderId, status: requestedStatus });
    } catch (error) {
      logger.error("adminUpdateOfflineOrderStatus failed", { error: error.message });
      return sendJson(res, error.status || 500, { ok: false, error: error.message || "Offline order status update failed" });
    }
  }
);
